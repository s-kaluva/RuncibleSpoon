...
body{width:100%;}
.txt-heading{padding: 5px 10px;font-size:1.1em;font-weight:bold;color:#DB4437;}
...
#shopping-cart table td{background-color:#eae4d8;}
...
<div style="background-color:#DB4437; width:85%; font-size:25px; color: white;">Shopping Cart<a id="btnEmpty" href="cart.php?empty=1"><img src="images/cartempty.png"></img></a></div>
<table style="text-align:center; border-right: 5px solid " border="1">
<tbody style=" font-size:20px; border: 3px solid #DB4437; ">
				<tr style="border: 3px solid #DB4437;">
				...
				<th><center><strong>Action</strong></center></th>
								...
								<td style= "background-color:#eae4d8; padding:1%" data-thumb="'.$row['c_image'].'">
								...
								?>	
					<tr>
					<?php $sql = " SELECT sum(c_price) AS total FROM cart ";
							$result = mysqli_query($conn, $sql);
							if (mysqli_num_rows($result) > 0) {
								while($row = mysqli_fetch_assoc($result)) {
				...
				<td class="txt-heading" colspan="3" align="right"><strong>Total = </strong>$  <?php if($item_total==0) echo '0'; else echo $item_total; ?></td>
				<td><a href="order.php?p='<?php echo $row["c_p_id"];?>'">
					<img src="images/cartodrnw.png" width="100" height="40"></img></a></td>
			</tr>
</tbody>
</table>	
</div>
